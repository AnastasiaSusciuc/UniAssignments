...
public:
    ...
    /*
     * description: adds the edge from node 'source' to node 'target' with the cost 'cost' in the graph
     *
    ...
    std::vector <std::pair <int, int> > parse_edges_out_with_costs(int node);
    std::vector <std::pair <int, int> > parse_edges_in_with_costs(int node);
