...
int Walks::count_different_minimum_walks(int start, int end) {
    ...
    distance.resize(all_nodes.size() + 5);
    diff_paths.resize(all_nodes.size() + 5); // diff_paths[i] - numarul de cai diferite de la nodul start la i
...
HamiltonHeuristic::HamiltonHeuristic(DirectedGraph &gr) : graph{gr} {
    visited.resize(gr.get_number_nodes()+5);
    for (auto i: visited)
        visited[i] = false;
}

void HamiltonHeuristic::approximate_tsp_nearest_neighbour() {
    visited.resize(graph.get_number_nodes()+5);
    for (int i = 0 ; i <= graph.get_number_nodes(); i++)
        visited[i] = false;
    hamilton_cycle.clear();
    cycle_cost = 0;
    dfs_nearest_neighbour(1, 1, 0);
}

bool HamiltonHeuristic::dfs_nearest_neighbour(int original_vertex, int node, int cycle_length) {
    visited[node] = true;
    std::vector <std::pair<int, int> > edges = graph.parse_edges_out_with_costs(node);
    std::sort (edges.begin(), edges.end(), HamiltonHeuristic::lowest_cost);
...
// v*e
    for (auto edge: edges)
    {
        ...
        if (neighbour == original_vertex && cycle_length == graph.get_number_nodes()-1)
        {
            hamilton_cycle.push_back(node);
            cycle_cost += cost;
            return true;
        }
        else if (!visited[neighbour])
        {
            found_original_vertex = dfs_nearest_neighbour(original_vertex, neighbour, cycle_length+1);
...
bool HamiltonHeuristic::lowest_cost(std::pair<int, int> a, std::pair<int, int> b) {
    return (a.second<b.second);
}

std::vector<int> HamiltonHeuristic::get_hamilton_cycle() const {
    return hamilton_cycle;
}

int HamiltonHeuristic::get_hamilton_cycle_cost() const {
    return cycle_cost;
}

MinimumCycle::MinimumCycle(DirectedGraph &gr) : graph{gr} {

}

int MinimumCycle::get_min_cycle() {
    std::vector <int> all_nodes = graph.parse_through_vertices();
    ...
    {
        std::vector <int> parent(all_nodes.size());
        std::vector <int> distances(all_nodes.size(), 0x3f3f3f3f);
        std::queue <int> Q;
        Q.push(i);
        distances[i] = 0;
        parent[i] = 0;
        ...
        while (!Q.empty())
        {
            ...
            std::vector <int> all_neigh = graph.parse_edges_out(node);

            for (auto neigh: all_neigh)
            {
                if (distances[neigh] == 0x3f3f3f3f) // we have not been here before
                {
                ...
                {
                    if (parent[neigh] == node || parent[node] == neigh) continue;
                    if (distances[neigh]+distances[node]+1 < answer)
                    {
                        answer = std::min(answer, distances[neigh]+distances[node]+1);
                        std::cout << i << " " << neigh << '\n';
                    }
                }
            }
...
ComputeSCC::ComputeSCC(DirectedGraph &gr) : graph{gr} {

}

void ComputeSCC::compute_scc() {
    std::vector <int> low_link(graph.get_number_nodes());
    std::vector <int> visited(graph.get_number_nodes());
