...
class HamiltonHeuristic{
private:
    DirectedGraph& graph;
    std::vector <bool> visited;             // we keep track of every node that has been visited in the dfs
    std::vector <int> hamilton_cycle;       // the way all the nodes were visited in the hamiltonian cycle
    int cycle_cost = 0;                     // the cost obtained in the hamiltonian cycle

    static bool lowest_cost(std::pair <int, int>, std::pair <int, int> ) ; // comparing function for sorting the edges
    bool dfs_nearest_neighbour(int original_vertex, int node, int cycle_length);
    /*
     * description:     we parse through all the adjacent nodes, visiting first the closest ones
     *                  when we reach the starting vertex, with a valid cycle of length of n-1, it means we found an approximate TSP
     *                  if we can't go to any new vertex, we backtrack
     *
     * param1:          original_vertex - int - the node we started the hamiltonian cycle
     * param2:          node - int - the current node from the dfs
     * param3:          cycle_length - int - the length of the path from the original_vertex to node
     *
     * returns:         true - if we found an approximate TSP
     *
     * */

public:
    explicit HamiltonHeuristic(DirectedGraph& gr);
    void approximate_tsp_nearest_neighbour();
    /*
     * returns:     - computes the approximate TSP
     * */
    std::vector <int> get_hamilton_cycle() const;
    /*
     * returns:     - the way all the nodes were visited in the hamiltonian cycle
     * */
    int get_hamilton_cycle_cost() const;
    /*
     * returns:     - the approximate TSP cost
     * */
};
