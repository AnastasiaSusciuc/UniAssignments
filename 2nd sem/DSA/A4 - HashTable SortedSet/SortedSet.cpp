...
// theta(number_elements*MOD) - because of the iterator, next operation taking theta(MOD) time
{
...
// worst case: theta(number_elements*MOD) - when we need to resize
// total complexity: theta(1) amortized
{
...
// worst case: theta(number_elements) - all elements are on the same branch and we want to erase the last
...
// best case: theta(1)
// worst case: theta(number_elements) - all elements are on the same branch and we search the last one
// average: theta(1) amortized
// total complexity: O(number_elements)
{
    int position = hashFunction(elem);
